    ...
    public function table(Table $table): Table
    {
        return $table
            ...
            ->modifyQueryUsing(function(Quotation $quotation) {
                /**
                 * normal user only allow to view own quotation
                 */
                // if (auth('web')->user()->role == User::IS_USER) {
                //     return $quotation->where('sales_person_id', auth()->user()->id)->orderBy('id','desc');
                // }
                    ...
                    Tables\Actions\Action::make('Accept')
                        ...
                        ->action(
                            function (Quotation $quotation, QuotationService $quotationService, array $data) {
                                ...
                                $lead = $quotation->lead; // Assuming the 'lead' relationship exists in Quotation

                                // Create a new ActivityLog entry with the desired description
                                ActivityLog::create([
                                ...
                                // Optionally log activity for auditing
                                activity()
                                    ->causedBy(auth()->user())
                                    ->performedOn($lead)
                                    ->withProperties([

...
class QuotationResource extends Resource
{
                        ...
                        Select::make('package_group')
                            ...
                            ->afterStateUpdated(function (?string $state, Forms\Get $get, Forms\Set $set) {
                                if ($state) {
                                    ...
                                    $yearCount = self::getPackageYearCount($state);
                                    ...
                                    foreach ($topPriorityProducts as $product) {
                                        ...
                                        $mappedItems->push([
                                            ...
                                            'quantity' => str_starts_with($product->solution, 'software') || str_starts_with($product->solution, 'hardware')
                                                ? ($product->quantity ?? 1)
                            ...
                            ->afterStateUpdated(function (Forms\Get $get, Forms\Set $set, $state) {
                                ...
                                foreach ($items as $index => $item) {
                                    if (!empty($item['product_id']) && isset($products[$item['product_id']])) {
                                        ...
                                        if ($product->solution === 'hrdf' || str_starts_with($product->solution, 'software')) {
                        ...
                        Repeater::make('items')
                            ...
                            ->itemLabel(
                                function(?array $state, \Filament\Forms\Components\Repeater $component, string $uuid): \Illuminate\Support\HtmlString|string|null {
                                    \Log::info('itemLabel ENTERED v4', ['uuid' => $uuid, 'pid' => $state['product_id'] ?? null]);
                                    ...
                                    // Compute year from item position (bypasses Filament state issues with 'year' field)
                                    \Log::info('itemLabel solution check', ['solution' => $product->solution, 'code' => $product->code]);
                                    if (str_starts_with($product->solution, 'software')) {
                                        try {
                                            $allItems = data_get($component->getLivewire(), $component->getStatePath()) ?? [];
                                            \Log::info('itemLabel allItems', [
                                                'count' => is_countable($allItems) ? count($allItems) : 'not_countable',
                                                'type' => gettype($allItems),
                                            ]);
                                            $totalOccurrences = 0;
                                            ...
                                            \Log::error('itemLabel year badge error', ['msg' => $e->getMessage()]);
                                        }
                                    }
    ...
    //         if (str_starts_with($product?->solution ?? '', 'software') || $product?->solution == 'hardware') {
    ...
    public static function recalculateAllRows($get, $set, $field = null, $state = null): void
    {
        ...
        foreach ($items as $index => $item) {
            ...
            if (!$currentQuantity || $currentQuantity == 0) {
                ...
                } elseif (str_starts_with($product?->solution ?? '', 'software') || $product?->solution === 'hardware') {

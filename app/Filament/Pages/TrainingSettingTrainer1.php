...
class TrainingSettingTrainer1 extends Page
{
    ...
    public function mount()
    {
        ...
        $this->showCalendar = true; // Show calendar by default
    ...
    public function showDateSelection($sessionId)
    {
        ...
        $holidays = PublicHoliday::whereBetween('date', [$weekStart, $weekEnd])
            ->get()
            ->keyBy('date');
    ...
    // Generate Microsoft Teams meetings
    ...
    private function createMeetingForDay($session, $dayNumber, $date)
    {
        try {
            ...
            $meetingRequest = [
                ...
                'attendees' => [], // Could add training attendees here in future
            ...
            if ($meetingLink && $meetingLink !== 'N/A') {
                try {
                    ...
                    if (isset($responseBody['value']) && count($responseBody['value']) > 0) {
                        ...
                        // STEP 3: Enable automatic recording (CustomerCalendar pattern)
                        if ($onlineMeetingId) {
                            try {
                                $recordingPayload = [
                                ...
                                \Illuminate\Support\Facades\Log::info('✅ Step 3: Automatic recording enabled (Training Setting)', [
                                    'online_meeting_id' => $onlineMeetingId,
                                    'session' => $session->session_number,
                                    'day' => $dayNumber
                                ]);
                                ...
                                \Illuminate\Support\Facades\Log::error('❌ Step 3: Failed to enable automatic recording (Training Setting)', [
                                    'error' => $e->getMessage(),
                                    'online_meeting_id' => $onlineMeetingId,
                                    'session' => $session->session_number,
                                    'day' => $dayNumber,
    ...
    {
        // Check if there are existing sessions for the selected year
        $existingSessions = TrainingSession::where('year', $this->selectedYear)->exists();

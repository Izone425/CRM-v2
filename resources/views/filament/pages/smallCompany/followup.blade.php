<div class="p-4 bg-white rounded-lg shadow" wire:poll.5s>
    <div class="flex items-center justify-between">
        <div class="flex items-center">
            <h3 class="text-lg font-bold">Follow Up (1-24)</h3>
            @if ($this->getFollowUpSmallCompanyLeads()->count() > 0)
                <div x-data="{ isOpen: false, leadId: null }">
                    <form wire:submit.prevent="resetDoneCall">
                        <x-filament::modal width="2xl">
                            ...
                            <div class="gap-3 mt-3 d-flex justify-content-center align-items-center" style="display: inline-flex; align-self: center; gap: 3rem;">
                                <button
                                    type="button"
                                    x-on:click="$wire.resetDoneCall; isOpen = false"
    ...
    @if ($this->getFollowUpSmallCompanyLeads()->count() > 0)
        <div class="mt-2 overflow-y-auto" style="max-height: 263px">
            <table class="w-full text-sm border-collapse table-fixed" style="width: 100%;">
                ...
                <thead class="sticky top-0 bg-gray-100" style="z-index: 1;">
                    <tr>
                        ...
                        <th class="px-2 py-2 font-semibold text-center text-gray-700 border-b cursor-pointer"
                            wire:click="sortBy('company_size', 'followUpSmallCompanyLeads')">
                            Company<br> Size
                            @if ($sortColumnFollowUpSmallCompanyLeads === 'company_size')
                                <span>{{ $sortDirectionFollowUpSmallCompanyLeads === 'asc' ? '▲' : '▼' }}</span>
                            @endif
                        </th>

                        <th class="px-2 py-2 font-semibold text-center text-gray-700 border-b cursor-pointer"
                            wire:click="sortBy('call_attempt', 'followUpSmallCompanyLeads')">
                            Call<br> Attempt
                            @if ($sortColumnFollowUpSmallCompanyLeads === 'call_attempt')
                                <span>{{ $sortDirectionFollowUpSmallCompanyLeads === 'asc' ? '▲' : '▼' }}</span>
                            @endif
                        </th>

                        <th class="px-2 py-2 font-semibold text-center text-gray-700 border-b cursor-pointer"
                            wire:click="sortBy('pending_time', 'followUpSmallCompanyLeads')">
                            Pending<br> Time
                            @if ($sortColumnFollowUpSmallCompanyLeads === 'pending_time')
                                <span>{{ $sortDirectionFollowUpSmallCompanyLeads === 'asc' ? '▲' : '▼' }}</span>
                            @endif
                        </th>
                ...
                @foreach ($this->getFollowUpSmallCompanyLeads() as $lead)
